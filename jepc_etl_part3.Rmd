---
title: "ETL Project Part 2"
output: html_document
---

This is the second part of an ETL project that takes unformatted, raw data from 5 different types of XML files and converts that data into an easy to use part/diagram look-up application.  The first part entailed the aggregation of data from 5 different types of formats from thousands of XML files into a more manageable collection of 5 datasets.  Now that the raw data has been gathered, it's time to extract the relevant information from it and recombine it in an easy to use format.  The end goal is to form the foundation of an application that will display the diagram of any part upon entry of a part number.
...
The TL_M_C_L file (shortened to tl) contains unformatted information that relates part names to a specific subsection of the parts catalog.  Each catalog page displays a labeled diagram for one subsystem of each car; this file contains the name, the subsystem, and a subsystem index for each part number it represents.
...
The data in tl is unformatted and not usable in its current form, but the internal subsystem labels used to identify car make and system that each catalog page refers to is embedded in this data and simply needs to be extracted.  The code below isolates the part names and category names and removes any extraneous characters/info.
...
Removing extraneous characters and splitting part name and indexes into separate fields
...
Reassigning name of tl to finaltl to indicate that this data.frame is ready for the next stage.  Unnecessary, but when dealing with multiple output tables, it's something that I tend to do just for my own organization.
...
rm(tl);rm(c1);rm(c2);rm(coln)

```

The cat_m_c_l file (shortened to cat) contains names for the categories and subcategories embedded in the tl file above.  It also contains unformatted referential keys that relate these names to the categories above.
...
Separating main category names from subcategory names and recombining them into a normalized format
...
item_m_c_i_l (shortened to item) contains part numbers for every part in the Jaguar catalog. It also contains reference keys to tie the part numbers back to part names in tl and subcategories in cat.
...
Separating all of the raw data from the 'item' table, cleaning it, and reformatting it in a more useful way.  Messy coding here!  Don't look!  

Edit:  Looking back on it, this code could be a fair bit cleaner, but it works quickly enough for this relatively small dataset. The part numbers are embedded in a long character string and are demarcated by the characters 'pb_02' or 'pb_27'. Much of this could have been together, but at the time, I wasn't sure how to make that work.  Nevertheless, the code below extracts records in which the part numbers that are notated by 'pb_02' are saved to one table and those that are notated by 'pb_27' are saved to a different table.  Those two tables are formatted so that category, subcategory and part numbers are in separate, usable fields and are then recombined back into the same table.
...
names(y) <- c("partno")
itemx <- as.data.frame(cbind(itemx$m, itemx$c, itemx$itemno, y$partno))
rm(y)
...
'imagefiles' contains the image name for every parts diagram, the main category name, and the subcategory name for each page in the catalog.  
